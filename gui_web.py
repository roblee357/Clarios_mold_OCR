...
from key_poller import Key_listener
...
class Camera:
    ...
    def setupMoldButton(self):
        ...
        self.moldButtonWindow.after(1000, self.startListeningForBarcode)

    def startListeningForBarcode(self):
        print('listening for barcode')
        print(self.kl.scanned)
        if self.kl.scanned:
            self.showInstructionWindow()
            self.kl.reset()
        self.moldButtonWindow.after(50, self.startListeningForBarcode)

    def showInstructionWindow(self):
        print('clicked')
        self.instructionWindow = Toplevel(self.root,takefocus=True)
        self.instructionWindow = Toplevel(self.root,takefocus=True)
        self.instructionWindow.focus()
        self.instructionWindow.grab_set()  #for disable main window
        self.instructionWindow.attributes('-topmost',True)  #for focus on toplevel
        self.instructionWindow.focus_force()
        ...
        self.instructionWindowPanel.image = self.instruction_image
    ...
    def checkForNewImg(self):
        # take second element for sort
        def takeSecond(elem):
            return elem[1]
        def mostRecentFile():
            self.capturePath = 'Android_Web\\flask\\examples\\tutorial\\flaskr\\images'
            all_files = os.listdir(self.capturePath);
            file_times = []
            for file in all_files:
                tup = (file,time.time() - os.stat(os.path.join(self.capturePath,file)).st_ctime)
                file_times.append(tup)
            print(tup)
            return  tup
        mostRcnt = mostRecentFile()[0]
        newMostRcnt = mostRcnt
        while newMostRcnt == mostRcnt:
            newMostRcnt = mostRecentFile()[0]
            print('checking...')
        self.capturePath = os.path.join(os.getcwd(), self.capturePath)
        self.captureImagePath = os.path.join(self.capturePath,newMostRcnt)
        self.instructionWindow.withdraw()
        self.instructionWindow.after(1000, self.setupConfirmationWindow)
    ...
    def setupConfirmationWindow(self):
        ...
        print('self.captureImagePath',self.captureImagePath)
        # subprocess.check_call(['open',  self.captureImagePath])
        newsize = (1200,800)
        self.capture_image = PIL.Image.open( self.captureImagePath)
        self.capture_image = PIL.ImageTk.PhotoImage(image=self.capture_image.resize(newsize))
        self.confWindowPanel = Label(master = self.confWindow, image= self.capture_image)
        self.confWindowPanel.image = self.capture_image
